...
import { buildGameplaySceneModel, defaultRendererThemeTokens, gameplayWorldGrid } from "./gameplay-scene-model.js";
...
export class AeroPlayCanvasRenderer {
  constructor(options={}){
    ...
    this.canvas=null;this.app=null;this.appStarted=false;this.cameraEntity=null;this.fixedCamera=null;this.gameplayGridLayer=null;this.gameplayGuidanceLayer=null;this.gameplayTargetLayer=null;this.gameplayHazardGlowLayer=null;this.gameplayColliderOverlayLayer=null;this.pool=[];this.assetPools=new Map();this.assetPoolGeneration=-1;this.assetMaterials=new WeakMap();this.materialStates=new WeakMap();this.markerPool=[];this.markerPoolMode="none";this.feedbackPool=[];this.feedbackTextures=new Map();this.activeCount=0;this.overlayEntities=[];this.entityMaterials=new WeakMap();this.ownedMaterials=new Set();this.sliceVariantMaterials=new Map();this.hazardGlowEntity=null;this.hazardGlowMaterial=null;
  ...
  detach(){
    ...
    const app=this.app;this.removeGameplayLayers();this.app=null;this.appStarted=false;this.cameraEntity=null;this.pool=[];this.assetPools=new Map();this.markerPool=[];this.markerPoolMode="none";this.feedbackPool=[];this.feedbackTextures=new Map();this.overlayEntities=[];this.iconTexture=null;this.atlasRestorePending=false;this.activeCount=0;this.sceneDiagnostics=emptySceneDiagnostics();this.cursorDiagnostics=Object.freeze({instanceCount:0,assetId:"athlete-marker/sphere-v1",roles:Object.freeze([]),depthTest:true,depthWrite:true});
  ...
  renderGameplayFrame(frame){return this.renderGameplayScene(frame,null,null);}
  renderGameplayFrameWithCursors(frame,cursors,options){return this.renderGameplayScene(frame,cursors,options);}
  renderGameplayScene(frame,cursors,cursorOptions){this.integrateDebugCameraMotion();const model=buildGameplaySceneModel(frame,this.theme,this.tuning,this.testPresentationConfig,this.gameplayVisualExperimentConfig),cameraDeflection=normalizeCameraDeflection(frame?.cameraDeflection??null);if(this.app&&!this.destroyed&&!this.contextLost)this.updateProductionCameraParallax(cameraDeflection);else this.resetProductionCameraParallax();this.lastModel=model;if(this.gameplayAssetLoader.describe().state==="error")this.gameplayAssetLoader.activateFallback("preload_error");if(!this.app||this.destroyed||this.contextLost)return{status:this.describe(),model,...(cursors===null?{}:{cursorCount:0,roles:Object.freeze([])})};try{this.clearOverlayEntities();this.applyCamera(model);this.updateSceneObjects(model.objects,model.guidance);this.applyClearColor();const cursorResult=cursors===null?null:this.stageGameplayCursors(cursors,cursorOptions,false);this.manualTick();this.frameCount+=1;this.drawCount+=model.objects.length+(cursorResult?.cursorCount??0);this.state="running";return{status:this.describe(),model,...(cursorResult??{})};}catch(error){this.fail(error);return{status:this.describe(),model,...(cursors===null?{}:{cursorCount:0,roles:Object.freeze([])})};}}
  ...
  stageGameplayCursors(cursors,options,clear){normalizeCursorGrid(options?.grid);if(!Array.isArray(cursors)||cursors.length>12)throw new TypeError("Gameplay cursors cannot exceed 12 candidates");const min=Math.max(0,Math.min(1,Number.isFinite(options?.minConfidence)?options.minConfidence:0.5));const accepted=new Map();for(const cursor of cursors){if(!plainData(cursor)||Object.keys(cursor).length!==4||!cursorRoles.includes(cursor.role)||accepted.has(cursor.role)||![cursor.x,cursor.y,cursor.confidence].every(Number.isFinite)||cursor.x<0||cursor.x>1||cursor.y<0||cursor.y>1||cursor.confidence<min)continue;accepted.set(cursor.role,cursor);}
  ...
  acquireColliderOverlayEntity(object,index){
    ...
    while(pool.length<=index){
      ...
      const material=new pc.StandardMaterial();material.useLighting=false;material.emissive=new pc.Color(1,1,1);material.diffuse=new pc.Color(1,1,1);material.opacity=1;material.blendType=pc.BLEND_NORMAL;material.depthTest=true;material.depthWrite=false;material.cull=pc.CULLFACE_NONE;material.update();
  ...
  applyAftermathAppearance(object,entity){
    ...
    const appearance=object.appearanceColor??this.roleColor(object.role),rgba=colorTokenToRgba(appearance,[1,1,1,1]);
    const colorToken=mixColorTokens(appearance,"#ffffff",object.tintMix);
    const stateColor=rgbaToHex(colorTokenToRgba(colorToken,[1,1,1,1]));
    for(const record of records){
      const variant=this.sliceVariantMaterial(record,sliceSign);
      record.meshInstance.material=variant;
      const state={kind:"aftermath-slice",sliceSign,diffuse:stateColor,opacity:rgba[3]*object.alpha,blendType:rgba[3]*object.alpha<1?pc.BLEND_NORMAL:record.blendType,depthWrite:rgba[3]*object.alpha<1?false:record.depthWrite,depthTest:true,useLighting:false,cull:record.cull,diffuseMap:record.diffuseMap,emissiveMap:record.emissiveMap,opacityMap:record.opacityMap,diffuseMapChannel:record.diffuseMapChannel,emissiveMapChannel:record.emissiveMapChannel,opacityMapChannel:record.opacityMapChannel};
      if(materialStateIsUnchanged(variant,this.materialStates.get(variant),state))continue;
      variant.diffuse.set(rgba[0],rgba[1],rgba[2]);variant.emissive.set(rgba[0],rgba[1],rgba[2]);
      variant.opacity=rgba[3]*object.alpha;variant.blendType=state.blendType;variant.depthWrite=state.depthWrite;variant.depthTest=true;variant.useLighting=false;variant.cull=record.cull;
  ...
  /** Restore pooled materials after a slice entity is disabled (prevents cross-contamination when the next icon reuses the same pool slot). */
  restoreSliceMaterials(entity){
    const records=this.assetMaterials.get(entity);if(!records?.length)return;
    for(const record of records)if(record.meshInstance.material!==record.material){record.meshInstance.material=record.material;this.materialStates.delete(record.meshInstance.material);}
  }
  ...
  clearSceneObjects(){for(const entity of this.pool)entity.enabled=false;for(const entries of this.assetPools.values())for(const entity of entries)entity.enabled=false;for(const entry of this.feedbackPool)entry.root.enabled=false;this.activeCount=0;this.sceneDiagnostics=emptySceneDiagnostics();}
...
function mixColorTokens(from,to,mix){const a=colorTokenToRgba(from,[1,1,1,1]),b=colorTokenToRgba(to,[1,1,1,1]),t=clamp(Number(mix),0,1);return`#${[0,1,2].map((index)=>Math.round((a[index]+(b[index]-a[index])*t)*255).toString(16).padStart(2,"0")).join("")}`;}
function gridPositionForNormalized(x,y,z){return{x:GRID_LEFT+clamp(x,0,1)*gameplayWorldGrid.columns*GRID_COLUMN_PITCH,y:GRID_TOP-clamp(y,0,1)*gameplayWorldGrid.rows*GRID_ROW_PITCH,z};}
