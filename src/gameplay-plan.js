...
export function buildGameplayRenderPlan(frame, theme = defaultRendererThemeTokens, tuning = defaultRendererTuning) {
  ...
  const grid = Object.freeze({ x: tuning.gridInset, y: tuning.gridInset, width: 1 - tuning.gridInset * 2, height: 1 - tuning.gridInset * 2, columns: /** @type {4} */ (4), rows: /** @type {3} */ (3) });
  /** @type {AeroGameplayDrawCommand[]} */
  const commands = [];
  if (frame.presentation === "boxing_semantic_track") {
    addTrack(commands, tuning);
  } else {
...
/** @param {AeroGameplayDrawCommand[]} commands @param {AeroRendererTuning} tuning */
function addTrack(commands, tuning) {
  const width = tuning.laneWidth;
  const gap = 0.1;
  const leftX = 0.5 - gap / 2 - width;
  const rightX = 0.5 + gap / 2;
  commands.push(command("rect", "left", { x: leftX, y: 0.08, width, height: 0.84 }, 0.12, 1, null, false, 0, null));
  commands.push(command("rect", "right", { x: rightX, y: 0.08, width, height: 0.84 }, 0.12, 1, null, false, 0, null));
  commands.push(command("line", "neutral", { x: leftX, y: 0.78, width, height: 0.008 }, 0.68, 1, null, false, 1, null));
  commands.push(command("line", "neutral", { x: rightX, y: 0.78, width, height: 0.008 }, 0.68, 1, null, false, 1, null));
}
...
function addTarget(commands, frame, target, grid, theme, tuning) {
  ...
  const progress = clamp(1 - (target.beatCenterMs - frame.nowMs) / lead, 0, 1);
  const feedback = clamp(target.feedbackProgress ?? 0, 0, 1);
  ...
  const rects = targetRects(frame.presentation, target, grid, tuning.gridGap);
...
/** @param {AeroGameplayPresentation} presentation @param {AeroRenderableTarget} target @param {{x:number,y:number,width:number,height:number}} grid @param {number} gap @returns {AeroNormalizedRect[]} */
function targetRects(presentation, target, grid, gap) {
  if (presentation === "boxing_semantic_track" && target.kind !== "obstacle") {
    if (target.kind === "guard") return [{ x: 0.25, y: 0.68, width: 0.5, height: 0.16 }];
    const x = (target.lane ?? target.hand) === "left" ? 0.25 : 0.55;
    return [{ x, y: 0.7, width: 0.2, height: 0.2 }];
  }
  const cells = target.cells.length > 0 ? target.cells : target.cell === null ? [] : [target.cell];
  if (target.kind === "guard" && cells.length >= 2) {
    const first = cellRect(cells[0], grid, gap);
    const second = cellRect(cells[1], grid, gap);
  ...
  return cells.map((cell) => cellRect(cell, grid, gap)).filter((rect) => rect !== null);
}
