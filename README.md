...
- `setDebugCameraEnabled` and `resetDebugCamera` provide Test-only free-fly presentation controls. Right mouse requests pointer lock; mouse movement looks; `WASD`, `Q`/`E`, and Shift move the camera. Disabling, detaching, or destroying releases listeners and pointer lock.
