...
try{
  ...
  const evidence=await page.evaluate(()=>{
    ...
    const configIdentity={schema:"aerobeat/equipment_config_identity",version:1,algorithm:"sha256",value:"1".repeat(64)};
    const quat=(xDeg,yDeg,zDeg)=>{const x=xDeg*Math.PI/360,y=yDeg*Math.PI/360,z=zDeg*Math.PI/360,qx={x:Math.sin(x),y:0,z:0,w:Math.cos(x)},qy={x:0,y:Math.sin(y),z:0,w:Math.cos(y)},qz={x:0,y:0,z:Math.sin(z),w:Math.cos(z)},mul=(a,b)=>({x:a.w*b.x+a.x*b.w+a.y*b.z-a.z*b.y,y:a.w*b.y-a.x*b.z+a.y*b.w+a.z*b.x,z:a.w*b.z+a.x*b.y-a.y*b.x+a.z*b.w,w:a.w*b.w-a.x*b.x-a.y*b.y-a.z*b.z}),q=mul(mul(qz,qy),qx),n=Math.hypot(q.x,q.y,q.z,q.w);return{x:q.x/n,y:q.y/n,z:q.z/n,w:q.w/n};};
    const pose=(scale,rotation)=>({role:"left_wrist",mode:"flow",anchor:{x:1.5,y:1,z:.45},scale,orientation:quat(...rotation),geometryIdentity:"aerobeat/saber_capsule_v1",configIdentity});
    ...
    const rows=[];
    for(const [scale,rotation] of [[1,[0,0,0]],[2,[0,0,90]],[.75,[25,-20,35]]]){
      renderer.renderGameplayFrameWithCursorsAndEquipment(frame,[],null,[pose(scale,rotation)],null);const entries=renderer.equipmentPools.get("equipment/flow-saber-v1:left_wrist"),root=entries[0],model=entries.find((e)=>e.name==="equipment-left_wrist-model"),glow=entries.find((e)=>e.name==="equipment-left_wrist-glow"),records=renderer.assetMaterials.get(model)??[];
      ...
      const rootScale=root.getLocalScale(),rootRotation=root.getLocalRotation();rows.push({scale,rotation,rootScale:[rootScale.x,rootScale.y,rootScale.z],rootRotation:[rootRotation.x,rootRotation.y,rootRotation.z,rootRotation.w],shared:model.parent===root&&glow.parent===root,core:metrics(core),glow:metrics(halo),overlap:overlap(core,halo),poolCount:entries.length});
    }
