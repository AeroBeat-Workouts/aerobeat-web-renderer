...
assert.deepEqual(rendererVisualScaleBounds,{min:.1,max:2});
{
  ...
  assert.deepEqual({...normalizeRendererTuning({...defaultRendererTuning,hash:undefined}),hash:"visual-3fed1dee"},defaultRendererTuning,"0.0.63 D5: default tuning round-trips through the v6 normalization exactly (wider slice separation added)");
...
const pending=target("flow","pending-motion",1000,null);for(const nowMs of [999,1000,1100,1180]){const icon=buildGameplaySceneModel({...frame("flow",[pending]),nowMs},undefined,undefined,undefined,config()).objects.find((entry)=>entry.kind==="icon");assert.equal(icon?.position.z,nowMs===1000?0:(1000-nowMs)*-.006,`pending note continues through/past goal at ${nowMs}`);}const miss=target("flow","miss-motion",1000,null,{judgement:"miss",missCommitMs:1181});const missModel=buildGameplaySceneModel({...frame("flow",[miss]),nowMs:1181},undefined,undefined,undefined,config()),missIcon=missModel.objects.find((entry)=>entry.kind==="icon"),missLabel=missModel.objects.find((entry)=>entry.kind==="feedback");assert.equal(missIcon?.position.z,1.086);assert.equal(missLabel?.position.z,1.086);assert.equal(missIcon?.appearanceColor,"#2a3038");
