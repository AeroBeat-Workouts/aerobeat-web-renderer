  ...
  for(const [viewport,width,height] of [["portrait",390,844],["landscape",844,390]]) for(const requestedDpr of [1,3]){
    const perspective=await page.evaluate(({width,height,requestedDpr})=>{
      ...
      const sameTargets=[
        ...
        {...flowBase,id:"depth-far",beatCenterMs:3490},
      ...
      const obstacleAfterFeedback=renderer.renderGameplayFrame({presentation:"flow",nowMs:2351,overlay:"none",targets:[obstacle]});const obstacleAfterFeedbackPixels=read();const persistentPlanes=obstacleAfterFeedback.plan.commands.filter((entry)=>entry.targetId==="flow-plane");
      const obstacleWithFeedback=renderer.renderGameplayFrame({presentation:"flow",nowMs:2351,overlay:"none",targets:[{...obstacle,judgement:"hit",feedbackProgress:1}]});const obstacleWithFeedbackPixels=read();const feedbackPlanes=obstacleWithFeedback.plan.commands.filter((entry)=>entry.targetId==="flow-plane");let feedbackPixelDifference=0;for(let index=0;index<obstacleAfterFeedbackPixels.length;index+=1)if(obstacleAfterFeedbackPixels[index]!==obstacleWithFeedbackPixels[index])feedbackPixelDifference+=1;
      ...
      return{effectiveDpr:renderer.describe().devicePixelRatio,atlasCellSize:globalThis.__AERO_RENDERER_TEST__.atlasCellSize,fillOrder:fills.map((entry)=>entry.targetId),cuePixels,rings:rings.map((entry)=>({id:entry.targetId,rect:entry.rect,depth:entry.depth,scale:entry.scale})),ringOnlyChanged,approachPlanes,impactPlanes,persistentPlanes,feedbackPlanes,feedbackPixelDifference,obstaclePixels,impactPixels,crisp:{white,role,strongNeighborEdges,maxNeighborDelta,outline:crispOutline.rect,fill:crispFill.rect},grid:same.plan.grid};
    ...
    assert.equal(perspective.approachPlanes.length,2);assert.equal(perspective.impactPlanes.length,2);assert.ok(perspective.approachPlanes.every((entry)=>entry.alpha===.58&&entry.intervalStartMs===2000&&entry.intervalEndMs===2600));assert.ok(perspective.obstaclePixels.every((count)=>count>20)&&perspective.impactPixels.every((count)=>count>20),`${viewport} DPR${requestedDpr} obstacle planes must be visible`);assert.deepEqual(perspective.feedbackPlanes,perspective.persistentPlanes,`${viewport} DPR${requestedDpr} obstacle planes must ignore synthetic 350ms feedback state`);assert.equal(perspective.feedbackPixelDifference,0,`${viewport} DPR${requestedDpr} obstacle framebuffer must be independent of synthetic feedback`);assert.ok(perspective.feedbackPlanes.every((entry)=>entry.kind==="plane"&&entry.iconId===null),`${viewport} DPR${requestedDpr} obstacle must emit plane commands only`);assert.equal(perspective.feedbackPlanes.some((entry)=>entry.kind==="ring"||entry.iconId==="feedback.great"),false,`${viewport} DPR${requestedDpr} obstacle must not emit ring/GREAT`);assert.ok(perspective.crisp.white>20&&perspective.crisp.role>60&&perspective.crisp.strongNeighborEdges>10&&perspective.crisp.maxNeighborDelta>70,`${viewport} DPR${requestedDpr} 256px alpha mask arrow/outline edges must remain crisp: ${JSON.stringify(perspective.crisp)}`);
    perspectivePixelEvidence.push({viewport,requestedDpr,...perspective});
  }
  assert.equal(perspectivePixelEvidence.length,4);
  console.log("Perspective Flow full-horizon same-cell depth, destination ring, feedback-independent obstacle plane/no-ring/no-GREAT, and 256px crisp-mask framebuffer validation passed for requested DPR1/3 portrait/landscape.");
