...
{
  const {renderer,advance}=makeMotionRenderer();const canvas={style:{cursor:"crosshair"}};renderer.debugPosition={x:2,y:3,z:4};renderer.debugYaw=.25;renderer.debugPitch=-.2;renderer.debugProjection={verticalFovDegrees:72,nearClip:.2,farClip:140};renderer.debugCaptureMode="fallback";renderer.debugCaptureCursor={canvas,inline:"crosshair",computed:"crosshair"};renderer.debugKeyboardIntents.add("left");renderer.debugDomIntents.add("forward");renderer.debugShiftActive=true;renderer.debugTouchToggle={startedAt:0,points:new Map(),moved:false,maxTouches:2};const pose={position:{...renderer.debugPosition},yaw:renderer.debugYaw,pitch:renderer.debugPitch,projection:{...renderer.debugProjection}};renderer.setDebugCameraAuthoringInputEnabled(false);assert.deepEqual({capture:renderer.debugCaptureMode,cursor:canvas.style.cursor,keyboard:[...renderer.debugKeyboardIntents],dom:[...renderer.debugDomIntents],shift:renderer.debugShiftActive,touch:renderer.debugTouchToggle},{capture:"none",cursor:"crosshair",keyboard:[],dom:[],shift:false,touch:null},"disabling authoring input must release capture and clear movement state");assert.deepEqual({position:renderer.debugPosition,yaw:renderer.debugYaw,pitch:renderer.debugPitch,projection:renderer.debugProjection},pose,"disabling authoring input must retain debug camera pose and projection");renderer.setDebugCameraMovementIntent("forward",true);renderer.applyDebugLookDelta(10,10,.01);renderer.onDebugTouchStart({touches:[{identifier:1,clientX:0,clientY:0},{identifier:2,clientX:1,clientY:1}],timeStamp:0,preventDefault(){throw new Error("disabled touch input must be ignored");}});advance(100);assert.deepEqual({position:renderer.debugPosition,yaw:renderer.debugYaw,pitch:renderer.debugPitch,touch:renderer.debugTouchToggle},{position:pose.position,yaw:pose.yaw,pitch:pose.pitch,touch:null},"disabled DOM movement, look, and touch input must be ignored");renderer.setDebugCameraAuthoringInputEnabled(true);renderer.setDebugCameraMovementIntent("forward",true);renderer.applyDebugLookDelta(10,0,.01);advance(0);advance(100);assert.notDeepEqual(renderer.debugPosition,pose.position,"re-enabling authoring input must resume movement");assert.notEqual(renderer.debugYaw,pose.yaw,"re-enabling authoring input must resume look");assert.throws(()=>renderer.setDebugCameraAuthoringInputEnabled(0),/must be boolean/u);
}
{
