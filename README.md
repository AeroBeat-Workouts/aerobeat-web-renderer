...
- `setDebugCameraEnabled` and `resetDebugCamera` provide Test-only free-fly presentation controls. Right-click toggles pointer capture (with bounded fallback when pointer lock is unavailable); Escape, pointer-lock loss, or a second right-click exits. A two-finger tap toggles touch capture and captured one-finger drag looks. Held `WASD`, `Q`/`E`, and Shift integrate camera-relative movement only on caller-owned render frames.
- `setDebugCameraMovementIntent(intent, active)` accepts only `forward`, `back`, `left`, `right`, `down`, or `up` for accessible DOM hold controls. `setDebugCameraSpeedMode(mode)` accepts only `normal` or `boost`. Disabling, visibility loss, blur, detaching, or destroying clears all capture, keys, DOM holds, touches, movement timing, listeners, and pointer lock.
