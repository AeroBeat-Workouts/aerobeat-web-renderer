...
function targetObjects(frame,target,window,successZone,theme,tuning,presentationConfig,reach,aftermathTargetIds){
  ...
  const icons=aftermathHandoff?[]:targetVisible?iconPositions.map((p,index)=>{const kindScale=(target.kind==="bomb"||target.family==="bomb"?tuning.bombScaleFactor:tuning.noteScaleFactor)*removalScale;return sceneObject(`${target.id}:${index}`,"icon",role,target.id,{x:p.x,y:p.y,z},{x:kindScale,y:kindScale,z:kindScale},null,assetId,rotation,state==="hit"?Math.max(0,1-(removal?.progress??1)):1,state,null,Boolean(removal),null,null,z,10,pairKey,pairKey===null?null:index,removal,null,iconAppearanceColor);}):[];
  ...
  if(resolved&&target.kind!=="obstacle"&&target.kind!=="bomb"&&positions.length&&elapsedMs<tuning.feedbackDurationMs){
    ...
    const visual=Object.freeze({text:/** @type {"Great"|"Miss"} */(hit?"Great":"Miss"),holdMs:FEEDBACK_HOLD_MS,fadeMs:FEEDBACK_FADE_MS,totalMs:tuning.feedbackDurationMs,elapsedMs,alpha,faceColor:hit?"#ffffff":"#e5484d",separationColor:"#171a22",depthBias:0.01,apparentHeightCssPx:hit?GREAT_HEIGHT_CSS_PX:MISS_HEIGHT_CSS_PX,offsetX:motion.x,offsetY:motion.y,scale:motion.scale,animation:/** @type {"bounce"|"shake"} */(hit?"bounce":"shake")});
