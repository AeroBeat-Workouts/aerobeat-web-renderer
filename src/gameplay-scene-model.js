...
/** @typedef {{text:"Great"|"Miss",holdMs:number,fadeMs:number,totalMs:number,elapsedMs:number,alpha:number,faceColor:string,separationColor:string,depthBias:number}} AeroFeedbackVisual */
/** @typedef {{elapsedMs:number,durationMs:number,progress:number}} AeroRemovalVisual */
/** @typedef {{id:string,kind:"icon"|"obstacle"|"cell"|"lane"|"track"|"feedback",role:AeroVisualRole,targetId:string|null,position:AeroWorldPosition,scale:AeroWorldScale,rotationZRad:number,alpha:number,iconId:string|null,assetId:string|null,tintColor:string|null,tintMix:number,whiteCore:boolean,state:AeroSceneTargetState|null,transparent:boolean,intervalStartMs:number|null,intervalEndMs:number|null,sortDepth:number,renderOrder:number,guardPairKey:string|null,guardPairIndex:number|null,removal:AeroRemovalVisual|null,feedback:AeroFeedbackVisual|null}} AeroGameplaySceneObject */
...
const REMOVAL_MS=80,FEEDBACK_HOLD_MS=180,FEEDBACK_FADE_MS=170,MAX_FEEDBACK=4;
...
function targetObjects(frame,target,window,successZone,theme,tuning){
  ...
  if(flowObstacle){
    ...
    return{objects:[sceneObject(`${target.id}:wall`,"obstacle",role,target.id,{x:centerX,y:centerY,z:center},{x:scaleX,y:scaleY,z:depth},null,ASSET.wall,0,1,null,pulse,true,interval.startMs,interval.endMs,center,30,null,null,null)],feedback:[]};
  }
  ...
  const removal=resolved?Object.freeze({elapsedMs,durationMs:REMOVAL_MS,progress:clamp(elapsedMs/REMOVAL_MS,0,1)}):null;
  const targetVisible=!resolved||elapsedMs<REMOVAL_MS;
  ...
  const targetObjects=targetVisible?positions.map((p,index)=>sceneObject(`${target.id}:${index}`,"icon",role,target.id,{x:p.x,y:p.y,z},{x:tuning.roleScale*removalScale,y:tuning.roleScale*removalScale,z:tuning.roleScale*removalScale},null,assetId,rotation,resolved?Math.max(0,1-(removal?.progress??1)):1,state,tintMix,Boolean(removal),null,null,z,10,pairKey,pairKey===null?null:index,removal)):[];
  ...
  if(resolved&&target.kind!=="obstacle"&&target.kind!=="bomb"&&positions.length&&elapsedMs<tuning.feedbackDurationMs){
    const crossingZ=0;
    const alpha=elapsedMs<=FEEDBACK_HOLD_MS?1:clamp(1-(elapsedMs-FEEDBACK_HOLD_MS)/FEEDBACK_FADE_MS,0,1);
    const visual=Object.freeze({text:/** @type {"Great"|"Miss"} */(state==="hit"?"Great":"Miss"),holdMs:FEEDBACK_HOLD_MS,fadeMs:FEEDBACK_FADE_MS,totalMs:tuning.feedbackDurationMs,elapsedMs,alpha,faceColor:state==="hit"?"#ffffff":"#e5484d",separationColor:state==="hit"?"#171a22":"#ffffff",depthBias:0.01});
  ...
  return{objects:targetObjects,feedback:feedbackObjects};
}
