...
const inventory=JSON.parse(await readFile("assets/gameplay/0.0.10/inventory.v1.json","utf8"));
const assetFiles=[...inventory.payload.map(({path})=>`assets/gameplay/0.0.10/${path}`),"assets/gameplay/0.0.10/inventory.v1.json","assets/gameplay/0.0.10/proof.v1.json"];
const packageFiles=[
 "LICENSE.md","README.md","docs/decisions/.gitkeep","docs/decisions/0002-playcanvas-world-gameplay-renderer.md","docs/decisions/0003-pinned-gameplay-asset-loader.md","docs/decisions/0004-assembly-environment-lifecycle.md","docs/decisions/flow-direction-cue-visibility-debug.md","docs/decisions/flow-perspective-exact-endpoint-debug.md","docs/decisions/flow-perspective-projection.md","docs/decisions/per-game-gameplay-renderer.md","docs/decisions/world-view-handedness-migration.md","docs/decisions/uniform-wall-browser-oracle-debug.md","package.json","src/test-presentation-config.js","src/environment-asset-owner.js","src/gameplay-asset-loader.js","src/gameplay-assets.js","src/gameplay-camera-pose.js","src/gameplay-scene-model.js","src/icon-atlas.js","src/index.js","src/landmark-mapping.js","src/renderer-facade.js","src/visual-profiles.js",
 ...assetFiles
].sort();
...
assert.equal(metadata.entryCount,41);assert.equal(actual.filter((entry)=>entry.startsWith("assets/gameplay/0.0.10/")).length,17);assert.ok(actual.every((entry)=>!entry.endsWith(".blend")&&!entry.includes("/review/")&&!entry.includes("/0.0.1/")&&!entry.includes("/tools/")));assert.ok(actual.filter((entry)=>entry.endsWith(".glb")).every((entry)=>entry.startsWith("assets/gameplay/0.0.10/")),"package must contain no environment GLB");assert.equal(actual.filter((entry)=>/\.(?:jpe?g|png)$/u.test(entry)).length,0,"package must contain no environment image payload");
