...
- `setEnvironmentAsset(descriptor|null)` and `setEnvironmentVisible(boolean)` are a private assembly-wiring seam, not an AeroBeat host/public-contract extension. The exact deeply frozen descriptor pins the assembly-owned `luminious-ice-cave-photosphere` identity, same-origin packaged JPEG URL, MIME, bytes/SHA-256, equirectangular projection, dimensions, zero orientation, center-forward `−Z`, and world-up `+Y`. The loader verifies response URL/type/bytes/hash before locally decoding the already-fetched bytes, rejects decoded-dimension drift, creates one low-geometry Skybox-layer photosphere without changing fixed gameplay camera/coordinates, and keeps a ready hidden root resident without refetch.
...
Context loss removes the private atlas texture, pinned gameplay containers, and optional assembly-described photosphere resources. Context restoration starts fresh gameplay and environment generations; stale fetch/decode completion is rejected. The environment is one radius-30 outward-wound procedural sphere (16×32 bands, 1,024 triangles) in PlayCanvas's built-in Skybox layer. Its renderer-owned unlit/emissive JPEG texture uses opaque blending, front-face culling, disabled frustum culling, and disabled depth test/write, so it stays behind all gameplay including the track through world `z=−72`. UV geometry directly preserves source `−Z` center, `+Y` up, and left/right orientation without mirroring. The next caller-owned render recreates the atlas, avoiding stale extension state and autonomous recovery timers. `detach()` and synchronous idempotent `destroy()` abort pending fetch/decode work and release decoded image, sphere mesh, material, texture, listeners, entities, gameplay container assets, the application, and pointer lock. No engine RAF or non-package asset request is introduced.
